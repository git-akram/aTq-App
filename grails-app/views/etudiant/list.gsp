			...
			<table>
				<thead>
					<tr>
						...
						<g:sortableColumn property="login" title="${message(code: 'etudiant.login.label', default: 'Login')}" />
					
						<g:sortableColumn property="password" title="${message(code: 'etudiant.password.label', default: 'Password')}" />
					
						<g:sortableColumn property="isAdmin" title="${message(code: 'etudiant.isAdmin.label', default: 'Is Admin')}" />
				...
				<g:each in="${etudiantInstanceList}" status="i" var="etudiantInstance">
					<tr class="${(i % 2) == 0 ? 'even' : 'odd'}">
						...
						<td>${fieldValue(bean: etudiantInstance, field: "email")}</td>
					
						<td>${fieldValue(bean: etudiantInstance, field: "login")}</td>
					
						<td>${fieldValue(bean: etudiantInstance, field: "password")}</td>
					
						<td><g:formatBoolean boolean="${etudiantInstance.isAdmin}" /></td>

			...
			<table>
				<thead>
					<tr>
						...
						<g:sortableColumn property="libelle" title="${message(code: 'cours.libelle.label', default: 'Libelle')}" />
					
						<g:sortableColumn property="description" title="${message(code: 'cours.description.label', default: 'Description')}" />
				...
				<g:each in="${coursInstanceList}" status="i" var="coursInstance">
					<tr class="${(i % 2) == 0 ? 'even' : 'odd'}">
						...
						<td><g:link action="show" id="${coursInstance.id}">${fieldValue(bean: coursInstance, field: "libelle")}</g:link></td>
